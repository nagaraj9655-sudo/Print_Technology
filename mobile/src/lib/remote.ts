...
function billToRow(b: Bill) {
  return {
    ...
    gst_enabled: b.gstEnabled ?? null, gst_inclusive: b.gstInclusive ?? null, simple_bill: b.simpleBill ?? null, tax_invoice: b.taxInvoice ?? null, original_cost: b.originalCost ?? null, bill_type: b.billType ?? 'Online',
...
function rowToBill(r: any): Bill {
  return {
    ...
    gstEnabled: r.gst_enabled ?? undefined, gstInclusive: r.gst_inclusive ?? undefined, simpleBill: r.simple_bill ?? undefined, taxInvoice: r.tax_invoice ?? undefined, originalCost: r.original_cost ?? undefined, billType: r.bill_type ?? 'Online',
...
export async function adminDeleteUser(id: string) {
  // Try the Edge Function first (deletes the Auth user + profile in one call).
  const fn = await db().functions.invoke('admin-create-user', { body: { action: 'delete', id } }).catch((e) => ({ data: null, error: e }))
  // If the function ran and returned an application-level error, surface it.
  if (!fn.error && (fn.data as any)?.error) throw new Error((fn.data as any).error)
  // If the function succeeded with no error, we're done.
  if (!fn.error) return
  // Fallback for ANY Edge Function failure (not deployed, network error, 404, etc.):
  // Delete the profile row directly. The Supabase Auth user entry isn't removed
  // via this path, but the record disappears from the users list immediately.
...
function readFnError(error: unknown): string {
  ...
  if (ctx?.status === 404) return 'The admin-create-user function is not deployed yet.'
  return (error as Error)?.message ?? ''
}

/* ---------------- bulk load + first-run seed ---------------- */

export async function fetchAll(): Promise<Database> {
  const s = db()
